...
func _ready() -> void:
	...
	arrow_instance.attack_type = attack_type
	...
	var user = get_parent()
	if attack_type == "SINGLE_SHOT":	
	# Add it to the current scene
		user.get_tree().current_scene.add_child(arrow_instance)
	
	if attack_type == "RAPID_FIRE":
		for i in range(3):
			var rapid_arrow = arrow_scene.instantiate()
			rapid_arrow.direction = directions[sector]
			rapid_arrow.rotation = deg_to_rad(possible_rotations[sector] + 45)
			rapid_arrow.position = global_position
			user.get_tree().current_scene.add_child(rapid_arrow)
